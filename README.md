...
## Simple Shell
> We are going to code a simple Unix shell. The shell, command line interpreter will utilise the command line interface.
...
```getline```, ```close```, ```open```, ```read```, ```signal```, ```malloc```, ```free```, ```getcwd```, ```chdir```, ```access```, ```execve```, ```wait```, ```write```,  ```exit```

### Description of what each file shows:
```
man_3_shell ------------------------ custom manpage for the simple shell
main.c ----------------------------- initializes the program
main.h ---------------------------- prototypes of functions in all the files
```
Helper files
```
...
Install and Compile
```
(your_terminal)$ git clone https://github.com/teddykopudo/simple_shell.git
...
### To Do
* More functionality should be included 
